...
namespace DDZ
{
	...
	enum Category
	{
		...
		// 三连带单
		TripleChainPlusSingle,
		// 三连带对
		TripleChainPlusDouble,
...
#pragma region 发牌
			//	从input中随机抽出cnt张牌
			static std::vector<short> deal_cards(std::vector<short> & input, int cnt);
#pragma endregion 

#pragma region 提取牌型
			static Category trait_category(const std::vector<short>& input);
#pragma endregion 
